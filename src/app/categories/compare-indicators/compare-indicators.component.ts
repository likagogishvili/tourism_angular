...
export class CompareIndicatorsComponent implements OnInit {
  ...
  selMainIndic: IDropDownSt = { name: 'მიზანი', value: 'Goal', isDisabled: false };
  ...
  selHelpInidc: IDropDownSt = { name: 'ყველა', value: 'All', isDisabled: false };
  ...
  getCharts(): any{
    if(this.isValue == 1){
      ...
      this.getHelpChartForign(this.year, this.quarter, this.mIndic, this.hIndic);
      this.getHelpChartExit(this.year, this.quarter, this.mIndic, this.hIndic);
      this.getHelpChartLocale(this.year, this.quarter, this.mIndic, this.hIndic);
    }
  }
  ...
  selhelpIndicChange() {
    this.selHelpInidc = this.indicsHelp.filter(x => x.value == this.hIndic)[0];
    if (this.hIndic == "All" && this.mIndic == "All"){
      this.mIndic = "Visit";
      this.selMainIndic = this.indicsMain.filter(x => x.value == this.mIndic)[0];
    }
    am4core.disposeAllCharts();
    this.getCharts();
  }
  ...
  createChart(res: any, divClass: string) {
    ...
    // Create series
    function createSeries(this: any, field: string, name: string, chrt: am4charts.XYChart, indic: string, val: number) {
      ...
      if (indic != "Expence") {
        ...
        series.columns.template.tooltipText = '{name}: [bold]{valueX.formatNumber("#.0a")}[/]';
      }
      ...
      categoryLabel.tooltipText = '{name}: [bold]{valueX.formatNumber("#.%")}[/]';
