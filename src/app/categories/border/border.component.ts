...
export class BorderComponent implements OnInit {
  ...
  getVisitsChart(st: number, en: number, stM: number, enM: number) {
    if (this.tType == 1) {
      this.http.get<any>(this.APIUrl + '/countByBorders?start=' + st + '&end=' + en + '&startM=' + stM + '&endM=' + enM).subscribe(data => {
        this.helpChart(data, "", "topChart");
      })
    }
    else{
      this.http.get<any>(this.APIUrl + '/countByBordersExit?start=' + st + '&end=' + en + '&startM=' + stM + '&endM=' + enM).subscribe(data => {
        this.helpChart(data, "", "topChart");
      })
    }
  ...
  createSeries(field: string, name: string, chart: any, ragac: string) {
    ...
    if (this.tType == 1) {
      bullet.tooltipText = '[bold]{name}[/]ში შემომსვლელ ვიზიტორტა რაოდენობა, საწყის პერიოდთან შედარებით,\n{year} წელს შეიცვალა [bold]{valueY.formatNumber("#.%")}-ით';
    }
    else{
      bullet.tooltipText = '[bold]{name}[/]ში გამსვლელ ვიზიტორტა რაოდენობა, საწყის პერიოდთან შედარებით,\n{year} წელს შეიცვალა [bold]{valueY.formatNumber("#.%")}-ით';
    }
